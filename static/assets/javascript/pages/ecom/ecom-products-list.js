    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                ...
                var product_group = document.getElementById('id_product_group').value;
                
                var search_url = "/ecom-products-list?product_id=" + product_id + "&product_model=" + product_model+ "&product_group=" + product_group;
                table_data = $('#dt-table-list').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'fields.product_model' },
                        { data: 'fields.product_group' },
                        { data: 'fields.product_price' },
                        { data: 'fields.discount_amount' },
                        { data: 'fields.product_old_price' },
                        {
                            "data": null,
...
$(function () {
    ...
    $('#dt-table-list').on('click', 'button', function () {
        ...
        if (class_name == 'btn btn-info btn-sm') {
            window.open("/ecom-product-vieworedit/"+id);
        }
    ...
    $('#id_product_id').change(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data)
                // data[0].product_id
                $('#id_product_model').val(data[0].product_model)
                // $('#id_product_group').val(data[0].product_group)
                $('#id_product_price').val(data[0].product_sales_price)
            }
        })
    });
});
