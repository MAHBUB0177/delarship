    ...
    function () {
        ...
        _createClass(fn_data_table, [{
            ...
            value: function table() {
                var from_account_number = document.getElementById('id_from_account_number').value;
                var search_url = "/apifinance-deposittransfer-api/?from_account_number=" + from_account_number;
...
$(function () {
    $('#btnSearch').click(function () {
        var account_number = document.getElementById('id_from_account_number').value;
        if (account_number === "") {
...
$("#id_from_account_number").on("change", function () {
    get_client_info();
});
...
function cancel_transaction(id) {
    if (confirm('Are you sure you want to cancel this Transaction?') == true) {
        ...
        $.ajax({
            url: "/finance-deposittransfer-cancel/" + id,
            type: "POST",
            success: function (data) {
                if (data.form_is_valid) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.form_is_valid) {
                ...
                $('#page_loading').modal('hide');
                alert(data.success_message);
                var from_account_number = document.getElementById("select2-id_from_account_number-container");
                from_account_number.textContent = "-----------------";
                var to_account_number = document.getElementById("select2-id_to_account_number-container");
                to_account_number.textContent = "-----------------";
                var global_branch_code = document.getElementById('id_global_branch_code').value;
                $('#id_branch_code').val(global_branch_code);
            } else {
                $('#page_loading').modal('hide');
