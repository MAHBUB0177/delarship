...
$("#id_invoice_discount").on("change paste keyup", function () {
	var invoice_discount = roundNumber(Number(document.getElementById('id_invoice_discount').value));
	//var total_bill_amount = roundNumber(Number(document.getElementById('id_total_bill_amount').innerHTML));
	total_product_price = total_bill_amount-(invoice_discount+Number(total_discount_amount))
	document.getElementById("id_total_discount_amount").innerHTML = numberWithCommas(roundNumber(invoice_discount+Number(total_discount_amount)));
	document.getElementById("id_total_product_price").innerHTML = numberWithCommas(roundNumber(total_product_price));
	document.getElementById("id_total_due_amount").innerHTML = numberWithCommas(roundNumber(total_product_price));
});
...
$("#id_account_number").on("change paste keyup", function () {
	...
	$('#id_product_id').val('').trigger('change')
});
...
$("#id_product_id").on("change paste keyup", function (e) {
	...
	if (key == 13) // the enter key code
	{
		...
		if (phone_number === '') {
			...
			document.getElementById("id_type_code").disabled = true;
		}
		return false;
	}
});
...
$("#id_product_bar_code").on("change paste keyup", function (e) {
	...
	get_product_name_by_barcode(product_bar_code);
	var key = e.which;
	if (key == 13) // the enter key code
	{
...
function get_product_name_by_barcode(bar_code) {
	var account_number = document.getElementById('id_customer_id').value;
	var branch_code = document.getElementById('id_branch_code').value;
	var type_code = document.getElementById('id_type_code').value;
	$.ajax({
		url: "sales-product-price",
		data: {"product_id":"","bar_code":bar_code,"branch_code":branch_code,"account_number":account_number,"tran_type_code":type_code},
		type: 'GET',
		success: function (data) {
			if (data.form_is_valid) {
				$('#id_product_id').append($('<option>', {
					value: data.product_code,
					text: data.product_name
				}));
				$('#id_product_id').val(data.product_code).trigger('change')
				$('#id_product_name').val(data.product_name);
				$('#id_product_price').val(data.product_price);
				$('#id_discount_rate').val(data.discount_percent);
				$('#id_discount_amount').val(data.discount_amount);
...
$(function () {
	...
	$(function () {
		$('#btnClearItem').click(function () {
			if (confirm('Are you sure you want to Clear Your Chart?') == true) {
				...
				document.getElementById("id_type_code").disabled = false;
				document.getElementById("sales_details_form").reset();
	...
	function clear_chart_item() {
		var data_url = '/sales-details-clear';
		$.ajax({
			url: data_url,
			data: '',
			type: 'POST',
