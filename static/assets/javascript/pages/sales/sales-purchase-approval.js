    ...
    function () {
        ...
        _createClass(DataTablesResponsiveDemo, [{
            ...
            value: function table() {
                table_data = $('#dt-stock-approval').DataTable({
                    "processing": true,
                    "ajax": {
                        "url": "/stock-master-api/",
                        "type": "GET",
                        "dataSrc": ""
                    },
                    responsive: true,
...
$(function () {
    var id = 0
    $('#dt-stock-approval').on('click', 'button', function () {
        ...
        if (class_name == 'btn btn-secondary btn-sm') {
            view_stock_details(stock_id, stock_date)
        }
    })
    ...
    function reject_stock(stock_pk) {
        if (confirm('Are you sure you want to reject this stock?') == true) {
            ...
            $.ajax({
                url: "/sales-purchase-reject/" + stock_pk,
                type: "POST",
    ...
    function show_stock_payment_model(stock_pk) {
        $('#page_loading').modal('show');
        $.ajax({
            url: '/sales-purchase-payment/' + stock_pk,
            type: 'get',
            dataType: 'json',
            beforeSend: function () {
                $('#page_loading').modal('hide');
